...
void genTexture(int w,int h){
	...
	//打印opengl es的版本信息
	char* e = (char*)glGetString(GL_VENDOR);
	LOGD("gl_vendor: %s", e);
	e = (char*)glGetString(GL_RENDERER);
	LOGD("gl_renderer: %s", e);
	e = (char*)glGetString(GL_VERSION);
	LOGD("gl_version: %s", e);


	//构造纹理
	pixel = (char*)malloc(64*64*4);
	int i = 0;
	char *p = pixel;
	for(i = 0; i < 64*64*4; i=i+4){
//		p[i] = 0;
//		p[i+1] = 255;
//		p[i+2] = 0;
//		p[i+3] = 255;

		*(int *)p = 0xFF00FF00;//一种赋值技巧，通过给int赋值，一次性赋值四个byte。这里面要注意数据的存储格式（小端模式）问题
		p = p + 4;
		/*
			小端模式：数据的高位存储到内存的高地址。
			怎么判断数据的高低位呢？
			比如int i = 15。i的16进制形式是0x0000000F，所以15是数据的地位，他会存储到内存的低地址（小端模式）。判断数据高低位
			最好的方法就是把他转换成16进制，最靠近0x部分的就是数据的高位。
		*/
	}
	//memset(pixel, 122, 64*64*4);
...
/*用来获取opengl的状态，画完后恢复状态*/
void getSetPara(int i){
	GLint matrixMode;
	GLint texState;
	...
	if(i){
		...
		glGetIntegerv(GL_TEXTURE_2D,&texState);
		glGetIntegerv(GL_ACTIVE_TEXTURE,&currentTexture);
		glGetIntegerv(GL_TEXTURE_BINDING_2D,&bindingTexture);
		glGetIntegerv(GL_VERTEX_ARRAY,&verArrayState);
		glGetIntegerv(GL_TEXTURE_COORD_ARRAY,&texArrayState);

		glGetPointerv(GL_TEXTURE_COORD_ARRAY_POINTER,&texPointer);
		glGetIntegerv(GL_TEXTURE_COORD_ARRAY_SIZE,&texSize);
		glGetIntegerv(GL_TEXTURE_COORD_ARRAY_STRIDE,&texStride);
		glGetIntegerv(GL_TEXTURE_COORD_ARRAY_TYPE,&texType);

		glGetPointerv(GL_VERTEX_ARRAY_POINTER,&verPointer);
		glGetIntegerv(GL_VERTEX_ARRAY_SIZE,&verSize);
		glGetIntegerv(GL_VERTEX_ARRAY_STRIDE,&verStride);
		glGetIntegerv(GL_VERTEX_ARRAY_TYPE,&verType);
		...
		glVertexPointer(verSize, verType, verStride, &verPointer);
		glTexCoordPointer(texSize, texType,texStride, &texPointer);
	}
}
...
void render(int width, int height){
	...
	/*个人感觉以下三行写在genTexture里面就好了，初始化一次，不需要每帧都执行，但是切换到后台再回来后，纹理里面的值
	被改变了，只能每帧执行下面三行。个人感觉是不是切换到后台后opengl的context重建了，所以需要重新设置纹理，不是很明白*/
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 64, 64, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixel);
